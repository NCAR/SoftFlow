...
module mo_lw_solver
  ...
  use mo_rrtmgp_solver_kernels, only: lw_solver_noscat, lw_solver_noscat_quad
  implicit none
  private
  ...
  real(wp), parameter,                         &  
    dimension(max_gauss_pts, max_gauss_pts) :: & 
      ...
      !
      gauss_Ds  = RESHAPE([1.66_wp,         HUGE(1._wp),   HUGE(1._wp),   HUGE(1._wp), &  ! Diffusivity angle, not Gaussian angle  
                           1.18350343_wp, 2.81649655_wp,   HUGE(1._wp),   HUGE(1._wp), & 
                           1.09719858_wp, 1.69338507_wp, 4.70941630_wp,   HUGE(1._wp), & 
...
contains
  ...
  function lw_solver(ncol, nlay, ngpt, top_is_1, &
    ...
    real(wp), dimension(ncol,       ngpt), optional, target, &             ! "User"-supplied integration secants    
    ...
    select type (atmos)
      class is (ty_optical_props)
        ...
        if(present(Ds)) then 
          ...
          !  
          call lw_solver_noscat_quad(ncol, nlay, ngpt, top_is_1, & 
