...
!
! Description:  RT solver for RRTMGP_SW.  Performs 2-stream no-scattering, (future) 2-stream scattering, and
...
module mo_sw_solver
  ...
  use mo_rrtmgp_solver_kernels, only: sw_solver_noscat, sw_solver_2stream
  implicit none
  private
  ...
  function sw_solver(ncol, nlay, ngpt, top_is_1,           &
    ...
    select type (atmos)
      class is (ty_optical_props)
        ...
        ! Direct beam only 
        !
        call sw_solver_noscat(ncol, nlay, ngpt, top_is_1,&
                              atmos%tau, mu0,             &
                              flux_dir)
        ! No diffuse flux
        flux_up = 0._wp 
      ...
      class is (ty_optical_props_2str)
        ...
        !
        call sw_solver_2stream(ncol, nlay, ngpt, top_is_1,         &
