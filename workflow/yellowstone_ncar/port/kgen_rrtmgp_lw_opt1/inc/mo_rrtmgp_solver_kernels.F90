...
contains 
  ...
  !   Users provide a set of weights and quadrature angles (specified by 
  !
  ...
  subroutine lw_solver_noscat(ncol, nlay, ngpt, top_is_1, D, weight,                             &
    ...
    do igpt = 1, ngpt
      ...
      do ilev = 1, nlay
        tau_loc(:,ilev) = max(tau(:,ilev,igpt)*D(:,igpt), 2._wp * TINY(1._wp))
      end do
      ...
      if(top_is_1) then 
        call lw_transport_noscat_top1(ncol, nlay,  & 
                                 tau_loc, lay_source(:,:,igpt), lev_source_dec(:,:,igpt), lev_source_inc(:,:,igpt), & 
                                 sfc_emis(:,igpt), sfc_src(:,igpt), radn_up(:,:,igpt), radn_dn(:,:,igpt)) 
      else
        call lw_transport_noscat_topN(ncol, nlay,  & 
                                 tau_loc, lay_source(:,:,igpt), lev_source_inc(:,:,igpt), lev_source_dec(:,:,igpt), & 
                                 sfc_emis(:,igpt), sfc_src(:,igpt), radn_up(:,:,igpt), radn_dn(:,:,igpt)) 
      end if 
      !
...
! ---------------------------------------------------------------
  ...
  subroutine lw_transport_noscat_top1(ncol, nlay, & 
                                 tau, lay_source, lev_source_up, lev_source_dn, sfc_emis, sfc_src, & 
                                 radn_up, radn_dn) 
    ...
    real(wp), dimension(ncol,nlay  ), intent( in) :: lay_source   ! Planck source at layer average temperature [W/m2]
    real(wp), dimension(ncol,nlay+1), target, &
                                      intent( in) :: lev_source_dn, lev_source_up
                                        ! Planck source at layer edge for upward and downward radiation [W/m2]
                                        ! Includes spectral weighting that accounts for state-dependent frequency to g-space mapping
    ...
    real(wp), dimension(ncol,nlay+1), intent(out) :: radn_up, radn_dn ! Radiances [W/m2-str]
                                                                      ! Top level must contain incident flux boundary condition
    ! Local variables
    real(wp), dimension(ncol, nlay) :: trans
    integer :: ilev
    ! ---------------------------------------------------
    do ilev = 1, nlay 
      trans(:,ilev) = exp(-tau(:,ilev))
    end do 
    ...
    do ilev = 2, nlay+1
      radn_dn(:,ilev) = trans(:,ilev-1) * radn_dn(:,ilev-1) +  &
                            (1._wp - trans(:,ilev-1)) *                  &
                            lay_emission(lay_source(:,ilev-1),  &
                                         lev_source_dn(:,ilev), &
                                         tau(:,ilev-1), trans(:,ilev-1))
    end do
    ...
    do ilev = nlay, 1, -1
      radn_up(:,ilev) = trans(:,ilev  ) * radn_up(:,ilev+1) +  &
                             (1._wp - trans(:,ilev  )) *                 &
                             lay_emission(lay_source(:,ilev  ),  &
                                          lev_source_up(:,ilev), &
                                          tau(:,ilev  ), trans(:,ilev  ))
    end do
...
! ---------------------------------------------------------------
  ...
  subroutine lw_transport_noscat_topN(ncol, nlay,  & 
    ...
    ! previous level is up (+1)
    do ilev = nlay, 1, -1
      radn_dn(:,ilev) = trans(:,ilev  ) * radn_dn(:,ilev+1) +  &
                            (1._wp - trans(:,ilev  )) *                  &
                            lay_emission(lay_source(:,ilev  ),  &
                                         lev_source_dn(:,ilev), &
                                         tau(:,ilev  ), trans(:,ilev  ))
    ...
    ! previous level is down (-1)
    do ilev = 2, nlay+1
      radn_up(:,ilev) = trans(:,ilev-1) * radn_up(:,ilev-1) +  &
                             (1._wp - trans(:,ilev-1)) *                 &
                             lay_emission(lay_source(:,ilev-1),  &
                                          lev_source_up(:,ilev), &
                                          tau(:,ilev-1), trans(:,ilev-1))
