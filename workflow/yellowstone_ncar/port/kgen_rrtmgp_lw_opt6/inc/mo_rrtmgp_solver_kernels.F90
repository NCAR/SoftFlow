...
module mo_rrtmgp_solver_kernels
  ...
  use mo_rrtmgp_constants, only: pi, & 
                              cp_dry, grav ! Only needed for heating rate calculation 
  implicit none
  private 
...
contains 
  ...
  subroutine lw_solver_noscat(ncol, nlay, ngpt, top_is_1, D, weight,                             &
    ...
    do igpt = 1, ngpt
      ...
      !
      call lw_source_noscat(ncol, nlay, top_is_1, & 
                            lay_source(:,:,igpt), lev_source_dec(:,:,igpt), lev_source_inc(:,:,igpt), & 
...
! ---------------------------------------------------------------
  ...
  !
  ! ---------------------------------------------------------------
  subroutine lw_source_noscat(ncol, nlay, top_is_1, lay_source, lev_source_dec, lev_source_inc, tau, trans, source_dn, source_up)
    integer,                           intent(in) :: ncol, nlay
    logical,                           intent(in) :: top_is_1
    real(wp), dimension(ncol, nlay  ), intent(in) :: lay_source, & ! Planck source at layer center
                                                     tau,        & ! Optical path (tau/mu)
                                                     trans         ! Transmissivity (exp(-tau)) 
    real(wp), dimension(ncol, nlay+1), intent(in) :: lev_source_dec, lev_source_inc
                                                                   ! Planck source at levels (layer edges), 
                                                                   !   increasing/decreasing layer index 
    ...
    integer :: ilay
    real(wp), dimension(ncol,nlay) :: fact
    ! ---------------------------------------------------------------
    !
    ...
    do ilay = 1, nlay
      !
      ! Merge required because, formally, trans -> 1 as tau -> 0 but numerical representation 
      !   of exp(-tau) = 1. for small but non-zero tau. 
      !
      fact(:,ilay) = merge((1._wp - trans(:,ilay))/tau(:,ilay) - trans(:,ilay), & 
                           0._wp, trans(:,ilay) < 1._wp - spacing(1._wp))
    end do
    ...
    if(top_is_1) then 
      do ilay = 1, nlay
        source_dn(:,ilay) = (1._wp - trans(:,ilay)) * lev_source_inc(:,ilay+1) + & 
                            2._wp * fact(:,ilay) * (lay_source(:,ilay) - lev_source_inc(:,ilay+1))
        source_up(:,ilay) = (1._wp - trans(:,ilay)) * lev_source_dec(:,ilay  ) + & 
                            2._wp * fact(:,ilay) * (lay_source(:,ilay) - lev_source_dec(:,ilay  ))
      end do 
    else
      do ilay = 1, nlay
        source_dn(:,ilay) = (1._wp - trans(:,ilay)) * lev_source_dec(:,ilay  ) + & 
                            2._wp * fact(:,ilay) * (lay_source(:,ilay) - lev_source_dec(:,ilay  ))
        source_up(:,ilay) = (1._wp - trans(:,ilay)) * lev_source_inc(:,ilay+1) + & 
                            2._wp * fact(:,ilay) * (lay_source(:,ilay) - lev_source_inc(:,ilay+1))
      end do 
    end if     
