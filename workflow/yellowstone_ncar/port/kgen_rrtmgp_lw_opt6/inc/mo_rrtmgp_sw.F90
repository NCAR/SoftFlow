...
module mo_rrtmgp_sw
  ...
  public :: rrtmgp_sw_init, rrtmgp_sw_opt
...
contains
  ...
  function rrtmgp_sw_phys(k_dist, gas_concs, p_lay, t_lay, p_lev, &
                          ...
                          aerosols, col_dry, inc_flux, tsi_scaling) result(error_msg)
    ...
    if(present(aerosols)) then
      error_msg = rrtmgp_sw_opt(k_dist, gas_concs, p_lay, t_lay, p_lev, & 
                                ...
                                aer_props, col_dry, inc_flux, tsi_scaling)
    else
      error_msg = rrtmgp_sw_opt(k_dist, gas_concs, p_lay, t_lay, p_lev, & 
                                mu0, sfc_alb_dir, sfc_alb_dif, cloud_props, & 
                                allsky_fluxes, clrsky_fluxes,           &
  ...
  function rrtmgp_sw_core(ncol, nlay, nband, ngpt,                    &
    ...
    if(present(inc_flux)) then 
      gpt_flux_dir(:,top_lev,:) = inc_flux(:,:)     * spread(mu0(:), dim = 2, ncopies = ngpt) * tsi_scl
    else 
      gpt_flux_dir(:,top_lev,:) = inc_flux_def(:,:) * spread(mu0(:), dim = 2, ncopies = ngpt) * tsi_scl
    end if 
