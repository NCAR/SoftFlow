...
subroutine radiation_init(pbuf2d)
   ...
   ! parameterizations.  Add fields to the history buffer.
   ...
   use mo_rrtmgp_lw,    only: rrtmgp_lw_init
   use mo_rrtmgp_sw,    only: rrtmgp_sw_init
   ...
   call coefs_init(coefs_lw_file, kdist_lw)

   ngpt_lw = kdist_lw%get_ngpt()

   errmsg = rrtmgp_lw_init()
   if (len_trim(errmsg) > 0) call endrun(sub//': ERROR: '//trim(errmsg))

   ! Initialize kdist_sw object
   call coefs_init(coefs_sw_file, kdist_sw)

   ngpt_sw = kdist_sw%get_ngpt()

   errmsg = rrtmgp_sw_init(2)
   if (len_trim(errmsg) > 0) call endrun(sub//': ERROR: '//trim(errmsg))
...
subroutine radiation_tend( &
   ...
   use mo_optical_props,   only: ty_optical_props, ty_optical_props_2str
   ...
   use mo_rrtmgp_sw,       only: rrtmgp_sw => rrtmgp_sw_opt
   ...
   if (dosw .or. dolw) then
      ...
      if (dosw) then
         ...
         errmsg = cloud_sw%init_2stream(nday, nlay, ngpt_sw)
         if (len_trim(errmsg) > 0) &
            call endrun(sub//': ERROR: cloud_sw%init_2stream: '//trim(errmsg))
      ...
      if (dolw) then
         ...
         ! deallocate the memory.  Not sure whether Fortran will deallocate this
         ! memory when the local object goes out of scope.
         errmsg = cloud_lw%init_1scalar(ncol, nlay, ngpt_lw)
         if (len_trim(errmsg) > 0) &
      ...
      if (dosw) then
         ...
         errmsg = aer_sw%init_2stream(nday, nlay, nswbands)
         if (len_trim(errmsg) > 0) &
            call endrun(sub//': ERROR: aer_sw%init_2stream: '//trim(errmsg))
         ...
         do icall = N_DIAG, 0, -1
            ...
            if (active_calls(icall)) then
               ...
               errmsg = rrtmgp_sw( &
                  ...
                  fsw, fswc, aer_props=aer_sw)
               call t_stopf('rad_sw')

               if (len_trim(errmsg) > 0) then
                  print*, 'ERROR return from rrtmgp_sw at lchnk=', lchnk
                  call endrun(sub//': ERROR code returned by rrtmgp_sw: '//trim(errmsg))
               end if
      ...
      if (dolw) then
         ...
         ! allocate object for aerosol optics
         ! Same comment as above regarding deallocation of this memory.
         errmsg = aer_lw%init_1scalar(ncol, nlay, nlwbands)
         if (len_trim(errmsg) > 0) &
         ...
         do icall = N_DIAG, 0, -1
            ...
            if (active_calls(icall)) then
               ...
               if (len_trim(errmsg) > 0) &
                  call endrun(sub//': ERROR code returned by rrtmgp_lw: '//trim(errmsg))

               call t_stopf('rad_lw')

               ! Transform RRTMGP outputs
               call set_lw_diags()
