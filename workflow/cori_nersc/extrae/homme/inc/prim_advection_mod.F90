...
#define _B4B 1
! If you activate both the new barrier code and the overlap feature the code
! will hang.  So we are deactivating the overlap feature for now.
#undef OVERLAP
!#define OVERLAP
...
subroutine remap1(Qdp,nx,qstart,qstop,qsize,dp1,dp2,hybrid)
  ...
  if (vert_remap_q_alg == 1 .or. vert_remap_q_alg == 2) then
    ...
    if ( present(hybrid) .and. tracer_num_threads > 1 ) then 
...
!$omp    private(z1c,z2c,zv,dp_np1,dp_star,Qcol,zkr,ilev) &
!$omp    private(zgam,zhdp,h,zarg,rhs,lower_diag,diag,upper_diag,q_diag,tmp_cal,filter_code) &
  ...
  do j = 1 , nx
    do i = 1 , nx
      ...
      do q = qstart, qstop
        ...
        masso(1) = 0.
        do k = 1 , nlev
          masso(k+1) = masso(k) + Qdp(i,j,k,q) !Accumulate the old mass. This will simplify the remapping
        enddo
        do k = 1 , nlev
          ao(k) = Qdp(i,j,k,q) / dpo(k)        !Divide out the old grid spacing because we want the tracer mixing ratio, not mass.
        enddo
...
!DIR$ ATTRIBUTES FORCEINLINE :: integrate_parabola
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine Prim_Advec_Tracers_spelt(elem, spelt, deriv,hvcoord,hybrid,&
    ...
    if (rsplit==0) then
       ...
       enddo
       call bndry_exchangeV(hybrid,edgeAdv1,location='prim_advection_mod:1029')
       do ie=nets,nete
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine Prim_Advec_Tracers_fvm(elem, fvm, deriv,hvcoord,hybrid,&
    ...
    use fvm_line_integrals_mod, only : cslam_runairdensity
    use fvm_line_integrals_flux_mod, only: cslam_runflux
    use fvm_consistent_se_cslam, only: run_consistent_se_cslam
    use fvm_line_integrals_mod, only: fvm_mcgregor, fvm_mcgregordss, fvm_rkdss
    use fvm_line_integrals_mod, only : fvm_ideal_test, IDEAL_TEST_OFF, IDEAL_TEST_ANALYTICAL_WINDS
    use fvm_line_integrals_mod, only : fvm_test_type, IDEAL_TEST_BOOMERANG, IDEAL_TEST_SOLIDBODY
    ...
    if (rsplit==0) then
       ...
       enddo
       call bndry_exchangeV(hybrid,edgeAdv1,location='prim_advection_mod:1159')
       do ie=nets,nete
...
!------------------------------------------------------------------------------------
    ...
    if (tracer_transport_type == TRACERTRANSPORT_CONSISTENT_SE_FVM) then
       call run_consistent_se_cslam(elem,fvm,hybrid,deriv,dt,tl,nets,nete,hvcoord%hyai(1)*hvcoord%ps0) !new se-cslam coupling
...
!       call fvm_mcgregordss(elem,fvm,nets,nete, hybrid, deriv, dt*fvm_supercycling, 3)
      ...
      call cslam_runflux(elem,fvm,hybrid,deriv,dt,tl,nets,nete,hvcoord%hyai(1)*hvcoord%ps0,.FALSE.)
...
!    !------------------------------------------------------------------------------------
  ...
  enddo
  call bndry_exchangeV(hy,edgeveloc,location='prim_advection_mod:1792')
  do ie=nets,nete
...
!-----------------------------------------------------------------------------
  ...
  use element_mod    , only : element_t
  use derivative_mod , only : derivative_t, gradient_sphere, vorticity_sphere, limiter_optim_iter_full
  use derivative_mod , only :divergence_sphere => divergence_sphere_routine
  use edge_mod       , only : edgevpack, edgevunpack
  use bndry_mod      , only : bndry_exchangev
  use hybvcoord_mod  , only : hvcoord_t
    ...
    do ie = nets, nete 
      ...
      do q = qbeg, qend
        do k= kbeg, kend
          ...
          qmin_val(k)                  = minval(Qtens_biharmonic(:,:,k,q,ie))
          qmax_val(k)                  = maxval(Qtens_biharmonic(:,:,k,q,ie))
        enddo
...
!           elem(ie)%derived%psdiss_biharmonic(i,j)
            !$OMP SIMD COLLAPSE(2)
            !dir$ vector aligned
            do j=1,np
              do i=1,np
...
!JMD !$OMP BARRIER
  ...
  call bndry_exchangeV( hybrid , edgeAdvp1,location='prim_advection_mod:2603')
...
! nelemd
  ...
  if(DSSopt==DSSno_var)then
     call bndry_exchangeV( hybrid, edgeAdv,location='prim_advection_mod:2731')
  else
     call bndry_exchangeV( hybrid, edgeAdvp1,location='prim_advection_mod:2733')
  endif
...
!JMD!$OMP BARRIER
    ...
    call bndry_exchangeV( hybrid , edgeAdv,location='prim_advection_mod:3010')
...
#undef REMAP_TE
      ...
      if (ntrac>0) then
        ...
        call remap1(cdp,nep,1,ntrac-1,ntrac-1,dpc_star,dpc)
        do i=1,nep
          do j=1,nep
            do k=1,nlev
...
!         call remap_velocityCspelt(np1,dt,elem,fvm,hvcoord,ie)
        ...
        ! call remap1(cdp,nc,1,ntrac-1,ntrac-1,fvm%c(:,:,:,1,np1_fvm),dp)
        ...
        if (ntrac>0) then
          !
          ! if CSLAM surface pressure is not overwritten with SE cell-integrated values the monthly 
          ! average of psC becomes noisy (accumulation of round-off errors)
          !
          fvm(ie)%psC(1:nc,1:nc)  = subcell_integration(elem(ie)%state%ps(:,:,np1), np, nc, elem(ie)%metdet)/&
           ...
           !
           do k = 1, nlev
!              fvm(ie)%dp_fvm(1:nc,1:nc,k,n_fvm)=interpolate_gll2fvm_points(dp(:,:,k),deriv) !phl - teg test
           end do
           !
           !
           !
...
!phl - reg test                 psc(i,j)=sum(fvm(ie)%dp_fvm(i,j,:,n_fvm))! +  hvcoord%hyai(1)*hvcoord%ps0
                 !
           ...
           do k=1,nlev
              do j=1,nc
                 do i=1,nc
                    fvm(ie)%dp_fvm(i,j,k,n_fvm)=dpc(i,j,k) !!XXgoldyXX??
