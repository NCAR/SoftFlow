...
use derivative_mod, only : derivative_t, laplace_sphere_wk, vlaplace_sphere_wk, derivinit
use derivative_mod, only : divergence_sphere => divergence_sphere_routine
use derivative_mod, only : vorticity_sphere  => vorticity_sphere_routine
...
! Original use of qtens on left and right hand sides caused OpenMP errors (AAM)
   ...
   call t_startf('bndry_exchangeV.edgeq')
   call bndry_exchangeV(hybrid,edgeq,location='viscosity_mod:374')
   call t_stopf('bndry_exchangeV.edgeq')
...
enddo
call t_startf('bndry_exchangeV.edge1')
call bndry_exchangeV(hybrid,edge1,location='viscosity_mod:501')
call t_stopf('bndry_exchangeV.edge1')
do ie=nets,nete
   kptr=0
...
enddo
call t_startf('bndry_exchangeV.edge2')
call bndry_exchangeV(hybrid,edge2,location='viscosity_mod:571')
call t_stopf('bndry_exchangeV.edge2')
do ie=nets,nete
   kptr=0
...
subroutine neighbor_minmax(hybrid,edgeMinMax,nets,nete,min_neigh,max_neigh)
   ...
   call t_startf('bndry_exchangeS.edgeMinMax') 
   call bndry_exchangeS(hybrid,edgeMinMax,location='neighbor_minmax')
   call t_stopf('bndry_exchangeS.edgeMinMax') 
...
subroutine neighbor_minmax_start(hybrid,edgeMinMax,nets,nete,min_neigh,max_neigh)
   ...
   call t_startf('bndry_exchangeS_start.edgeMinMax')
   call bndry_exchangeS_start(hybrid,edgeMinMax,location='viscosity_mod:882')
   call t_stopf('bndry_exchangeS_start.edgeMinMax')
...
subroutine neighbor_minmax_finish(hybrid,edgeMinMax,nets,nete,min_neigh,max_neigh)
   ...
   call t_startf('bndry_exchangeS_finish.edgeMinMax')
   call bndry_exchangeS_finish(hybrid,edgeMinMax,location='viscosity_mod:902')
   call t_stopf('bndry_exchangeS_finish.edgeMinMax')
...
!$omp parallel do num_threads(vert_num_threads) 
    ...
    call t_startf('bndry_exchangeV.edgebuf') 
    call bndry_exchangeV(hybrid,edgebuf,location='viscosity_mod:976')
    call t_stopf('bndry_exchangeV.edgebuf') 
