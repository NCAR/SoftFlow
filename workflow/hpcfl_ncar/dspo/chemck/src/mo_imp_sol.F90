...
!!$      eps((/id_o3,id_no,id_no2,id_no3,id_hno3,id_ho2no2,id_n2o5,id_oh,id_ho2/)) = .0001_r8
      ...
      subroutine imp_sol( base_sol_full, reaction_rates_full, het_rates_full, extfrc_full, delt, &
                          ncol, lchnk, chnkpnts, chnk_beg, chnk_end )
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
      ...
      integer, intent(in) :: chnk_beg, chnk_end
...
!        print *, ' chnkpnts_loop start ', ii, ofl, ofu
         avec_len = (ofu - ofl) + 1
...
!        print *, ' chnkpnts_loop return ', ii, ofl, ofu
         base_sol_full(ofl:ofu,:) = base_sol_blk(1:avec_len,:)
         ind_prd_full(ofl:ofu,:) = ind_prd_blk(1:avec_len,:)

         ofl = ofu + 1
!        if( ofl > chnkpnts ) then
         if( ofl > chnk_end ) then 
!           base_sol_full(ofe:ofu,:) = base_sol_blk(1:avec_len,:)
!           ind_prd_full(ofe:ofu,:) = ind_prd_blk(1:avec_len,:)
!           print *, ' chnkpnts_loop exit ', ii, ofl, ofu
            exit chnkpnts_loop
         end if
