...
def gen_plotpages(idx, typestr, scale_x, x_unit, papimin, papimax, papis_app, papis_kernel):

    ticks_x = ticker.FuncFormatter(lambda x, pos: '{0:g}'.format(x/scale_x))

    pdf = PdfPages('papi_lapack_%s.pdf'%typestr)
    ...
    axapp.set_xlabel('%s cache misses %s'%(typestr, x_unit), fontsize=LABEL_SIZE)
    ...
    axkernel.set_xlabel('%s cache misses %s'%(typestr, x_unit), fontsize=LABEL_SIZE)
...
def gen_report():
    ...
    # plot pages
    for idx in range(3):
        gen_plotpages(idx, typestr[idx], scale_x[idx], x_unit[idx], cfg['papimin'][idx], cfg['papimax'][idx],
