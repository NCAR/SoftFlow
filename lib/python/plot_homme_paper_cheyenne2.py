...
def gen_plotpages():
    ...
    fig.suptitle('         original(ipcc02)                      optimized(dungeon28)', fontsize=SUBTITLE_SIZE, y=0.95)
    ...
    for row in range(NROW):
        ...
        for col in range(NCOL):
            ...
            inner = gridspec.GridSpecFromSubplotSpec(1, 2, subplot_spec=outer[row, col], wspace=0.1, hspace=0.3)

            counter = selected_counters[row]
            plt.figtext( 0.05, 0.87 - row*0.138, counter, fontsize=LABEL_SIZE, rotation='vertical' )
            plt.figtext( 0.00, 0.5, '# events ($\mathregular{10^{6}}$)', fontsize=LABEL_SIZE, rotation='vertical' )
...
#                        ax.set_xlabel(counter, fontsize=LABEL_SIZE)
                ...
                for absfolddir, regionname in cfg['regions'].items():
                    if regionname == region:
                        ...
                        msecs.append(cfg['etimes'][absfolddir])
                        break
    ...
    plt.figtext( 0.15, 0.07, 'elapsed time (%s msec)          elapsed time ( %s msec)'%tuple(msecs), fontsize=SUBTITLE_SIZE )
