...
def gen_plotpages():
    ...
    axapp.set_title(cfg['title'], fontsize=TITLE_SIZE)
    axapp.set_xlabel('elapsed time (sec)')
    #plt.legend(loc='upper right')
    pdf.savefig(fig)

    for filepath, kernel_data in cfg['kernels'].items():
        ...
        content, _ = os.path.basename(filepath).split('M.txt')
        _, _, ranks, cache = content.split('_')
        #ranks = '1'
        #cache = '0'
        s = '' if int(ranks) == 1 else 's'
        #axkernel.set_title('MG2 kernel run (%s rank%s, %s MB)'%(ranks, s, cache), fontsize=TITLE_SIZE)
        axkernel.set_title('kernel run (%s rank%s, %s MB)'%(ranks, s, cache), fontsize=TITLE_SIZE)
