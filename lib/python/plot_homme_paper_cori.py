...
def gen_plotpages():
    ...
    # KNL
    selected_counters = ( ( 'PAPI_L1_DCA', 'PAPI_L1_DCM' ), ( 'PAPI_L2_TCA', 'PAPI_L2_TCM' ), ( 'OFFCORE_RESPONSE_0:MCDRAM_NEAR', 'OFFCORE_RESPONSE_0:MCDRAM_FAR' ) )
    ...
    for row in range(NROW):
        for col in range(NCOL):
            ...
            for idx, (region, (_xvals, yvals)) in enumerate(reversed(sorted(plotdata.items()))):
                ...
                #if region == 'original':
                #    #plt.legend(funcs, labels, loc='upper center', ncol=3, borderaxespad=-0.1)
                #    plt.legend(funcs, labels, loc='upper center', ncol=3)

                fig.add_subplot(ax)

    #fig.legend(funcs, labels, loc='lower center', ncol=3, borderaxespad=-0.1)
    funcs = []
    labels = []
