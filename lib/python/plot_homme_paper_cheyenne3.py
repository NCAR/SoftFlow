...
def gen_plotpages():
    ...
    selected_counters = ( 'PAPI_L1_LDM', 'PAPI_L2_DCR', 'PAPI_L3_DCR' )
    ...
    #plt.figtext( 0.3, 0.05, 'elapsed time (msec)', fontsize=LABEL_SIZE, ha='center', va='bottom' )
    #plt.figtext( 0, 0, 'elapsed time (msec)', fontsize=LABEL_SIZE, ha='center', va='bottom' )
    #plt.figtext( 0.3, 0.1, 'elapsed time (msec)', fontsize=SUBTITLE_SIZE )

    for row in range(NROW):

        counter = selected_counters[row]

        plt.figtext( 0.03, 0.85 - row*0.28, '# %s events ($\mathregular{10^{6}}$)'%counter, fontsize=LABEL_SIZE, rotation='vertical' )
...
#                        ax.set_xlabel('elapsed time (msec)', fontsize=LABEL_SIZE)
                ...
                #if row == NROW-1:
                #    ax.set_xlabel('elapsed time (msec)', fontsize=SUBTITLE_SIZE)
    ...
    #fig.legend(funcs, labels, loc='lower center', ncol=1, borderaxespad=-0.5, frameon=False, fontsize=SUBTITLE_SIZE)
    #fig.legend(funcs, labels, loc=8, ncol=3, borderaxespad=-0.5, frameon=False, fontsize=SUBTITLE_SIZE)
    fig.legend(funcs, labels, loc=8, ncol=3, fontsize=SUBTITLE_SIZE)
